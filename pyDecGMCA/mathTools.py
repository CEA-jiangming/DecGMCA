...
def fft2d1d(cube, shift=False):
    ...
    for fm in range(nz):
        if shift:
            ...
            cubefft[fm] = scifft.fftshift(scifft.fft2(scifft.ifftshift(cube[fm])))
    return cubefft
...
def ifft2d1d(cubefft, shift=False):
    ...
    for fm in range(nz):
        if shift:
            ...
            cube[fm] = scifft.fftshift(scifft.ifft2(scifft.ifftshift(cubefft[fm])))
    return cube
...
def fftNd1d(image, N, shift=False):
    if N == 1:
    ...
    for fm in range(nz):
        if N == 1:
            if shift:
                ...
                imagefft[fm] = scifft.fftshift(scifft.fft(scifft.ifftshift(image[fm])))
        if N == 2:
            if shift:
                imagefft[fm] = scifft.fft2(image[fm])
            else:
                imagefft[fm] = scifft.fftshift(scifft.fft2(scifft.ifftshift(image[fm])))
    return imagefft


def ifftNd1d(imagefft, N, shift=False):
    if N == 1:
        (nz, ny) = np.shape(imagefft)
    ...
    for fm in range(nz):
        if N == 1:
            if shift:
                ...
                image[fm] = scifft.fftshift(scifft.ifft(scifft.ifftshift(imagefft[fm])))
        if N == 2:
            if shift:
                image[fm] = scifft.ifft2(imagefft[fm])
            else:
                image[fm] = scifft.fftshift(scifft.ifft2(scifft.ifftshift(imagefft[fm])))
    return image
...
##########################################################################
# Particular tools for GMCA algorithm
##########################################################################
