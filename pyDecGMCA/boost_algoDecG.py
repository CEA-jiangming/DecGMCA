...
from pyDecGMCA.boost_utils import *
from pyDecGMCA.boost_Prox import *
...
def DecGMCA(V, M, n, Nx, Ny, Imax, epsilon, epsilonF, Ndim, wavelet, scale, mask, deconv, wname='starlet', thresStrtg=2,
            FTPlane=True, fc=1./16, logistic=False, postProc=0, postProcImax=50, Kend=3.0, Ksig=3.0, positivityS=False,
            positivityA=False):
    """
    ...
    for i in range(Imax):
        ...
        if FTPlane:
            if wavelet:
                ...
                A = UpdateA(V, Shat, M, mask=mask, deconv=deconv)
        else:
            A = UpdateA(V, S, M, mask=mask, deconv=deconv)
    ...
    if postProc == 1:
        ...
        for i in range(postProcImax):
            ...
            if FTPlane:
                ...
                A = UpdateA(V, A, S, M, inImax2, mu2, mask=mask, positivity=positivityA)
...
##########################################################################
def update_S_prox(V, A, S, M, Nx, Ny, Imax, mu, Ksig, mask, wavelet, scale, wname='starlet', thresStrtg=2,
                  FTPlane=True, FISTA=False):
    """
...
def update_S_prox_anal(V, A, S, M, Nx, Ny, Imax, Imax1, mu, mu1, Ksig, mask, wavelet, scale, wname='starlet',
    ...
    @param Imax: Number of global iterations
    
    @param Imax1: Number of sub-iterations
