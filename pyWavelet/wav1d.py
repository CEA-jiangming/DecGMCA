...
def scaleFilter(wname, wtype):
    if wtype == 1:
        if wname == 'haar' or wname == 'db1':
            ...
            sys.exit("Filter not yet implemented!")
        return F
    elif wtype == 2:
...
def wavFilters(wname, wtype, mode):
    if wtype == 1:
        ...
        [Lo_D, _, _, Hi_R, _, Hi_D, Lo_R, _] = biorWavFilter(Rf, Df)
    else:
        sys.exit("Filter not yet implemented!")
    if mode == 'd':
        return Lo_D, Hi_D
...
def wavOrth1d(x, nz, wname='haar'):
    ...
    band = np.zeros(scale + 1).astype(int)
    band[-1] = len(x)
...
def iwavOrth1d(wt, band, wname='haar'):
    if wname == 'haar' or wname == 'db1' or wname == 'db2' or wname == 'db3' or wname == 'db4' or wname == 'db5':
        wtype = 1
    else:
        wtype = 2
    ...
    start = band[0]
    for sc in range(np.size(band) - 2):
...
class Starlet1D(object):
    """
    Class of one-dimensional starlet

    List of attributes:
    self.scale, self.nele, self.fast, self.gen2, self.normalization, self.trTab

    List of methods:
    decomposition, reconstruction, adjoint
    """

    def __init__(self, nele, scale, fast=True, gen2=False, normalization=False):
        self.scale = scale
        self.nele = nele
        self.fast = fast
        self.gen2 = gen2
        ...
        sig = np.zeros(nele)
        sig[nele // 2] = 1
        wt = self.decomposition(sig, computewt=False)
        tmp = wt ** 2
        self.trTab = np.sqrt(np.sum(tmp, 1))

    def decomposition(self, sig, computewt=True):
        wt = np.zeros((self.scale, self.nele))
        step_hole = 1
        sig_in = np.copy(sig)
        ...
        wt[self.scale - 1] = np.copy(sig_out)
        if self.normalization and computewt:
    ...
    def reconstruction(self, coef):
        ...
        if self.gen2:
            ...
            sig = np.copy(wt[self.scale - 1])
            for k in range(self.scale - 2, -1, -1):
                if self.fast:
                    kernel1d = b3spline_fast(step_hole)
                    sig_pad = np.lib.pad(sig, (2 * step_hole, 2 * step_hole), 'reflect')
                    sig_out = psg.convolve(sig_pad, kernel1d, mode='valid')
                else:
                    sig_out = b3splineTrans(sig, step_hole)
                sig = sig_out + wt[k]
                step_hole //= 2
        else:
            '''
