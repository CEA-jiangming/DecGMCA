...
def main(args):
    ...
    components = ['21cm', 'ff', 'gsync', 'ptr']

    projectPath = '../../EoR/N400/'
    skymapPath = 'skymap/'
    dataPath = 'image'
    typeWgt = 'natural'

    # Read sky maps, visibility and beam
    Y = np.zeros((len(freqs), Nx, Ny)).astype('complex64')
    kern = np.zeros((len(freqs), Nx, Ny))
    kernmax = np.zeros(len(freqs))
    cube21cm = np.zeros((len(freqs), Nx, Ny))
    cube = np.zeros((len(freqs), Nx, Ny))

    for i, f in enumerate(freqs):
        for cmp in components:
            if cmp == '21cm':
                visname1 = join(projectPath, dataPath, cmp + '_' + typeWgt,
                                'eor_' + '0{:.2f}_N400-uv-real.fits'.format(f))
                visname2 = join(projectPath, dataPath, cmp + '_' + typeWgt,
                                'eor_' + '0{:.2f}_N400-uv-imag.fits'.format(f))
                kernname = join(projectPath, dataPath, cmp + '_' + typeWgt,
                                'eor_' + '0{:.2f}_N400-uvpsf-real.fits'.format(f))
                kerntmp = fits.getdata(kernname).squeeze()
                kernmax[i] = kerntmp.max()
                kern[i] = kerntmp
                skyname = join(projectPath, skymapPath, cmp,
                                'eor_' + '0{:.2f}_N400.fits'.format(f))
                cube21cm[i] = fits.getdata(skyname)
            else:
                visname1 = join(projectPath, dataPath, cmp + '_' + typeWgt,
                                cmp + '_0{:.2f}_N400-uv-real.fits'.format(f))
                visname2 = join(projectPath, dataPath, cmp + '_' + typeWgt,
                                cmp + '_0{:.2f}_N400-uv-imag.fits'.format(f))
                skyname = join(projectPath, skymapPath, cmp,
                               cmp + '_0{:.2f}_N400.fits'.format(f))
            tmp = fits.getdata(skyname)
            cube[i] += tmp
            vistmp = fits.getdata(visname1) + 1j * fits.getdata(visname2)
            Y[i] += vistmp.squeeze()
        kern[i] /= kernmax[i]
        Y[i] /= kernmax[i]
    ...
    if DECGMCA:
        ...
        (S_est_ori, A_est, deltaY, thIter) = DecGMCA(Y, kern, n, Nx, Ny, Imax, epsilon, epsilonF, Ndim,
                                 ...
                                 logistic=logistic, postProc=postProc, postProcImax=postProcImax, Ksig=Ksig, positivityA=positivity, positivityS=positivity, mixCube=None, csCube=cube21cm)

        fits.writeto('estS_ori_sdc_simu.fits', S_est_ori, overwrite=True)
        fits.writeto('estA_sdc_simu.fits', A_est, overwrite=True)

        # Residuals computed from estimated A,S and ground-truth mixtures
        X_ori = A_est @ S_est_ori.reshape(n, -1)
        res_dec_ori = cube.reshape(bd, -1) - X_ori
        fits.writeto('res_ori_sdc_simu.fits', res_dec_ori, overwrite=True)
        ...
        fits.writeto('estS_sdc_simu.fits', S_est, overwrite=True)
        X = A_est@S_est.reshape(n, -1)
        res_dec = cube.reshape(bd, -1) - X
        res_dec = res_dec.reshape((bd, Nx, Ny))
        fits.writeto('res_sdc_simu.fits', res_dec, overwrite=True)
        fits.writeto('thIter_sdc_simu.fits', thIter, overwrite=True)

        # Beamed ground-truth
        cube21cm_vis = np.real(ifft2d1d(fft2d1d(cube21cm) * kern.reshape(bd, Nx, Ny)))
        res_vis = np.real(ifft2d1d(deltaY))
        fits.writeto('res_vis_sdc_simu.fits', res_vis, overwrite=True)
        fits.writeto('21cm_beamed_sdc_simu.fits', cube21cm_vis, overwrite=True)
        ...
        fits.writeto('res_deconv_sdc_simu.fits', dec_eor, overwrite=True)
        ...
        if thresStrtg == 2:
            ...
            # plt.show()
            plt.savefig(
                'DecGMCA_sdc_simu_threshold_n{0}_scale{1}_thStrtg{2}_epsilonF{3}_kend{4}.png'.format(n, scale, thresStrtg, epsilonF, kend))
    ...
    if DECGMCA:
        # Compare residuals computed from estimated A, S with the ground-truth
        ...
        plt.ylabel('coef')
        plt.savefig('DecGMCA_sdc_simu_coef_ori_n{0}_scale{1}_thStrtg{2}_epsilonF{3}_kend{4}.png'.format(n, scale, thresStrtg, epsilonF, kend))
        ...
        plt.ylabel('coef')
        plt.savefig(
            'DecGMCA_sdc_simu_coef_n{0}_scale{1}_thStrtg{2}_epsilonF{3}_kend{4}.png'.format(n, scale, thresStrtg, epsilonF, kend))
        ...
        plt.ylabel('coef')
        plt.savefig(
            'DecGMCA_sdc_simu_vis_coef_n{0}_scale{1}_thStrtg{2}_epsilonF{3}_kend{4}.png'.format(n, scale, thresStrtg, epsilonF, kend))
        ...
        plt.ylabel('coef')
        plt.savefig(
            'DecGMCA_sdc_simu_dec_coef_n{0}_scale{1}_thStrtg{2}_epsilonF{3}_kend{4}.png'.format(n, scale, thresStrtg, epsilonF,
                                                                                       kend))
