...
def createKernel(t_samp, sigma, bd=1, export=True):
    ...
    print("The standard deviation of PSFs (" + str(bd) + " channels, " + str(t_samp) + " samplings) range from " + str(
...
def createNoise(Nx, Ny, n, sigS, db, numTests=1, export=True):
    ...
    for i in np.arange(numTests):
        ...
        N = N.squeeze()
        if export:
            fits.writeto(dr + 'noise_db' + str(int(db)) + '_sigS' + str(int(np.round(sigS))) + '_r' + str(i) + '.fits',
                         N, clobber=True)
