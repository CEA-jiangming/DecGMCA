...
import pylab
import simu_CS_deconv.param as param
import os
import sys
    ...
    for n in nArr:
        ...
        for r in np.arange(numTests):
            ...
            for pc in pcArr:
                ...
                #                                  wavelet=wavelet, scale=4, mask=mask, deconv=deconv, wname='starlet',
                #                                  thresStrtg=thresStrtg, FTPlane=FTPlane, fc=fc, logistic=logistic,
                #                                  postProc=postProc, postProcImax=postProcImax, Ksig=Ksig,
                #                                  positivityS=positivityS, positivityA=positivityA)
                #
                #     elif MC_GMCA_flag and mask:
                #         Xe = SVTCompletion(V_N, M, n, 1.5, 1000)
                #         (S_est, A_est) = GMCA(Xe, n, Nx, Ny, Imax, Ndim, wavelet=True, scale=4, wname='starlet',
                #                               thresStrtg=2, FTPlane=FTPlane, fc=fc, Ar=A)
                #     else:
                #         (S_est, A_est) = DecGMCA(V_N, MMat, n, Nx, Ny, Imax, epsilon[j], epsilonF, Ndim,
                #                                  wavelet=wavelet, scale=4, mask=mask, deconv=deconv, wname='starlet',
                #                                  thresStrtg=thresStrtg, FTPlane=FTPlane, fc=fc, logistic=logistic,
                #                                  postProc=2, postProcImax=postProcImax, Ksig=Ksig,
                #                                  positivityS=positivityS, positivityA=positivityA)
                ...
                # Ae = Ae.astype('float64')
                    (S_est, A_est) = GMCA_PALM(V_N, MMat, n, Nx, Ny, Imax, Ndim, wavelet, scale=4, mask=mask,
                                               deconv=deconv, wname='starlet', thresStrtg=thresStrtg, FTPlane=FTPlane,
                                               fc=fc, logistic=False, Ksig=Ksig, positivity=False, S=None, A=None)

                    fits.writeto(drResults + drReal + 'estS_bd' + str(b) + '.fits', S_est, overwrite=True)
                    fits.writeto(drResults + drReal + 'estA_bd' + str(b) + '.fits', A_est, overwrite=True)
