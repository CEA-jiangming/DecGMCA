...
#
# N = fits.getdata(
#     drNoise + 'n' + str(n) + '/' + 'noise_db' + str(int(db)) + '_sigS' + str(int(sigS)) + '_r' + str(r) + '.fits')
# N = N.astype('float64')
...
# V_N = (V + N) * MMat
#
# (S_est, A_est) = GMCA_class(V_N, kernMat, n, Nx, Ny, Imax, tauF, epsilon[j], Ndim, wavelet=True, scale=4, mask=mask,
#                             wname='starlet', coarseScaleStrtg=1, thresStrtg=2, FTPlane=FTPlane, fc=fc, Ar=A)
# X = ForWaRD(V_N, kernMat, epsilon=1e-3)
# (S_est, A_est) = DecGMCA(V_N, H, n, Nx, Ny, Imax, epsilon, epsilonF, Ndim, wavelet=True, scale=5, mask=mask,
#                          deconv=deconv, wname='starlet', thresStrtg=2, FTPlane=FTPlane, fc=fc, postProc=False, Ksig=0.6,
#                          positivity=False)
...
for Ksig in np.arange(3.0, 3.1):
    Se_postProc, thIter = update_S_prox_Condat_Vu(V_N, Ae, Se, MMat, Nx, Ny, Ndim, 50, tau=0, eta=1.0, Ksig=Ksig,
                                                  wavelet=True, scale=5, wname='starlet', thresStrtg=2, FTPlane=FTPlane,
                                                  positivity=False)
    # (S_est, A_est) = MC_GMCA(V_N, MMat, n, Nx, Ny, Imax, tauF, 0, Ndim, wavelet=True, scale=4, mask=False,
    #                          wname='starlet', coarseScaleStrtg=1, thresStrtg=2, FTPlane=FTPlane, fc=fc, Ar=A)
