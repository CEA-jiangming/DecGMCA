            ...
            for Ksig in np.arange(0.6, 0.7, 0.2):
                ...
                for r in np.arange(numTests):
                    ...
                    if deconv:
                        ...
                        # S = S.astype('float64')
                        #
                        # N = fits.getdata(drNoise + 'n' + str(n) + '/' + 'noise_db' + str(int(db)) + '_sigS' + str(
                        #     int(sigS)) + '_r' + str(r) + '.fits')
                        # N = N.astype('float64')
                    ...
                    for b in bd[-1:]:
                        ...
                        if deconv:
                            ...
                            # N = fits.getdata(drResult + subdr + drReal + 'noise_db' + str(int(db)) + '_sigS' + str(
                            #     int(np.round(sigS))) + '_bd' + str(b) + '_r' + str(r) + '.fits')
                        ...
                        # Se_post, thIter = update_S_prox(V_N, Ae, Se, M, Nx, Ny, Imax, mu=1., mask=True, wavelet=True,
                        #                                 scale=4, wname='starlet', coarseScaleStrtg=1, thresStrtg=2,
                        #                                 FTPlane=True, Ar=None, FISTA=False)
                        # Se_post, thIter = update_S_prox_anal(V_N, Ae, Se, kern, Nx, Ny, Imax, Imax1=30, mu=0.5, mu1=0.5,
                        #                                      Ksig=Ksig, mask=True, wavelet=True, scale=4,
                        #                                      wname='starlet', coarseScaleStrtg=1, thresStrtg=2,
                        #                                      FTPlane=True, Ar=None)
                        if mask:
                            Se_post, thIter = update_S_prox_Condat_Vu(V_N, Ae, Se, MMat, Nx, Ny, Ndim, Imax, tau=0.5,
                                                                      eta=0.5, Ksig=Ksig, wavelet=True, scale=4,
                                                                      wname='starlet', thresStrtg=2, FTPlane=True,
                                                                      Ar=None)
                        if deconv:
                            Se_post, thIter = update_S_prox_Condat_Vu(V_N, Ae, Se, kernMat, Nx, Ny, Ndim, Imax, tau=0.5,
                                                                      eta=0.5, Ksig=Ksig, wavelet=True, scale=4,
                                                                      wname='starlet', thresStrtg=2, FTPlane=True,
                                                                      Ar=None)
